...
type InfoComplementariaTercero struct {
	...
	InfoComplementariaTerceroPadreId *InfoComplementariaTercero `orm:"column(info_complementaria_tercero_padre_id);rel(fk)"`
}
...
func GetAllInfoComplementariaTercero(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(InfoComplementariaTercero))
	// query k=v
	for k, v := range query {
